...
| **Measure**  | ~~No `client_type` for MCP~~ **CORRECTED 2026-07-02:** attribution already exists — `MinimalAnalyticsService.detect_client_type` maps `oilpriceapi-mcp` → `client_type='sdk-mcp'` (+ semver via `detect_sdk_info`). The original finding was an artifact of a `LIMIT 12` breakdown query (sdk-mcp's 81 reqs fell below postman's 96). Remaining gap is reporting only: no loop/dashboard surfaces the sdk-mcp cohort. | 30d: 81 req / 8 users with `client_type='sdk-mcp'`                                                            |
...
### WS2 — Attribution (Measure) — ~~P0~~ RESOLVED as already-implemented (2026-07-02)

5. ~~Backend PR to parse the MCP UA~~ — **not needed.** `client_type='sdk-mcp'` + `sdk_version` already flow in production (oilpriceapi-api#3885 closed with evidence). No backend change, no deploy.
6. Add the sdk-mcp cohort to whatever dashboard/loop reads client_type today (oilpriceapi-api#3887 — unblocked, reporting-only).
...
| 3   | ~~Attribution: UA→client_type~~ **already implemented** | oilpriceapi-api | 0h     | **P0**   | DONE 2026-07-02: prod rows show `client_type='sdk-mcp'` (81/30d); #3885 closed with evidence                |
...
| 10  | MCP cohort in analytics/daily loop                      | oilpriceapi-api | 2h     | P2       | Loop report includes sdk-mcp row (unblocked — data already flows)                                           |

**Sequencing:** Day 1 = #1 #2 (+#6 drafts) — item #3 discovered already-implemented. Day 2 = #4 #5 → ship v2.4.0. Day 3 = #7 #8, then #9 #10 as capacity allows. No backend deploy needed for attribution.
